...
describe('#getDoorControl(...) (TCP)', function () {
  ...
  it('should execute get-door-control using TCP with address:port object', function (done) {
    ...
    uhppoted.getDoorControl(ctx, { id: 405419896, address: { address: '127.0.0.1', port: 59998 }, protocol: 'tcp' }, 3)
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
  ...
  it('should execute get-door-control using TCP with address:port string', function (done) {
    ...
    uhppoted.getDoorControl(ctx, { id: 405419896, address: '127.0.0.1:59998', protocol: 'tcp' }, 3)
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
