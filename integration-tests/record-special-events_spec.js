...
const request = Buffer.from([
  0x17, 0x8e, 0x00, 0x00, 0x78, 0x37, 0x2a, 0x18, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
describe('#recordSpecialEvents(...)', function () {
  ...
  it('should execute record-special-events', function (done) {
    ...
    uhppoted
      .recordSpecialEvents(ctx, 405419896, true)
      .then((response) => {
...
describe('#recordSpecialEvents(...) (TCP)', function () {
  ...
  it('should execute record-special-events using TCP with address:port string', function (done) {
    ...
    uhppoted
      .recordSpecialEvents(ctx, { id: 405419896, address: '127.0.0.1:59998', protocol: 'tcp' }, true)
      .then((response) => {
        expect(response).to.deep.equal(expected)
        done()
      })
