...
describe('#refreshTaskList(...) (TCP)', function () {
  ...
  it('should execute refresh-task-list with address:port object', function (done) {
    ...
    uhppoted.refreshTaskList(ctx, { id: 405419896, address: { address: '127.0.0.1', port: 59998 }, protocol: 'tcp' })
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
  ...
  it('should execute refresh-task-list with address:port string', function (done) {
    ...
    uhppoted.refreshTaskList(ctx, { id: 405419896, address: '127.0.0.1:59998', protocol: 'tcp' })
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
