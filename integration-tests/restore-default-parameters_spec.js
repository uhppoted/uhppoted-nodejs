...
describe('#restoreDefaultParameters(...) (TCP)', function () {
  ...
  it('should execute restore-default-parameters with address:port object', function (done) {
    ...
    uhppoted.restoreDefaultParameters(ctx, { id: 405419896, address: { address: '127.0.0.1', port: 59998 }, protocol: 'tcp' })
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
  ...
  it('should execute restore-default-parameters with address:port string', function (done) {
    ...
    uhppoted.restoreDefaultParameters(ctx, { id: 405419896, address: '127.0.0.1:59998', protocol: 'tcp' })
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
