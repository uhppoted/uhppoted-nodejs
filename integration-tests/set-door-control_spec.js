...
const request = Buffer.from([
  0x17, 0x80, 0x00, 0x00, 0x78, 0x37, 0x2a, 0x18, 0x03, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
describe('#setDoorControl(...)', function () {
  let sock = null
  ...
  it('should execute set-door-control', function (done) {
    const expected = {
      deviceId: 405419896,
      doorControlState: {
        door: 3,
    ...
    uhppoted
      .setDoorControl(ctx, 405419896, 3, 4, 'normally closed')
      .then((response) => {
...
describe('#setDoorControl(...) (TCP)', function () {
  ...
  it('should execute set-door-control using TCP with address:port string', function (done) {
    ...
    uhppoted
      .setDoorControl(ctx, { id: 405419896, address: '127.0.0.1:59998', protocol: 'tcp' }, 3, 4, 'normally closed')
      .then((response) => {
        expect(response).to.deep.equal(expected)
        done()
      })
