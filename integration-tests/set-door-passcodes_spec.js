...
describe('#setDoorPasscodes(...)', function () {
  ...
  it('should execute set-door-passcodes', function (done) {
    ...
    uhppoted.setDoorPasscodes(ctx, 405419896, 3, [12345, 0, 999999, 54321])
      .then(response => {
...
describe('#setDoorPasscodes(...) (TCP)', function () {
  ...
  it('should execute set-door-passcodes with address:port object', function (done) {
    ...
    uhppoted.setDoorPasscodes(ctx, { id: 405419896, address: { address: '127.0.0.1', port: 59998 }, protocol: 'tcp' }, 3, [12345, 0, 999999, 54321])
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
  ...
  it('should execute set-door-passcodes with address:port string', function (done) {
    ...
    uhppoted.setDoorPasscodes(ctx, { id: 405419896, address: '127.0.0.1:59998', protocol: 'tcp' }, 3, [12345, 0, 999999, 54321])
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
