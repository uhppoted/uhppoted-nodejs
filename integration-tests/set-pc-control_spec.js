...
describe('#setPCControl(...) (TCP)', function () {
  ...
  it('should execute set-pc-control with address:port object', function (done) {
    ...
    uhppoted.setPCControl(ctx, { id: 405419896, address: { address: '127.0.0.1', port: 59998 }, protocol: 'tcp' }, true)
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
  ...
  it('should execute set-pc-control with address:port string', function (done) {
    ...
    uhppoted.setPCControl(ctx, { id: 405419896, address: '127.0.0.1:59998', protocol: 'tcp' }, true)
      .then(response => {
        expect(response).to.deep.equal(expected)
        done()
      })
